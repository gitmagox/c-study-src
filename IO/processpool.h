...
{
	//保存所有子进程的描述信息
	struct mgx_process* m_sub_process;
	//进程池中进程的总数量
	...
	while( ! mgx_processpool->m_stop )
	{
		...
		for( int i=0; i< number; i++ )
		{
			...
			if( sockfd == mgx_processpool->m_listenfd )
			{
				...
				send( mgx_processpool->m_sub_process[i].m_pipefd[0], (char*)&new_conn, sizeof(new_conn), 0 );
...
processpool* mgx_process_create( int listendfd, int process_number)
{
	...
	//分配内存
	mgx_processpool->m_sub_process = (mgx_process *)malloc( process_number*sizeof(mgx_process) );
